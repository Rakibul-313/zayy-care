...
type ShopProduct = {
  ...
  active?: boolean;
  bestSeller?: boolean;
  flashSale?: boolean | string | number;
...
const SHOP_CACHE_KEY = "zayy_flash_sale_products_cache";
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        const flashSaleProducts = parsed.filter((product) => {
          const endTime = Number(product.flashSaleEndAt || 0);
          const isNotExpired = endTime === 0 || endTime > Date.now();
        ...
        setMaxPrice(
          Math.max(...flashSaleProducts.map((p) => p.price || 0), 1000)
        );
    ...
    const unsubscribe = onValue(ref(database, "products"), (snapshot) => {
      ...
      const formatted: ShopProduct[] = Object.entries(data)
        .map(([firebaseId, value], index) => {
          ...
          const flashSalePrice = Number(product.flashSalePrice || 0);
          ...
          return {
            ...
            flashSalePrice,
            flashSaleEndAt: Number(product.flashSaleEndAt || 0),
          };
        })
        .filter((product) => {
          const endTime = Number(product.flashSaleEndAt || 0);
            ...
            <div className="mx-auto max-w-[1820px] px-4 py-12 sm:px-8 md:py-16 lg:px-14 lg:py-20">
              <div className="relative z-10 max-w-[560px]">
                ...
                <p className="mt-4 max-w-[460px] text-[16px] leading-8 text-[#263421] sm:text-[17px]">
                    Grab limited-time deals on authentic Korean skincare before they expire.
                </p>
              </div>
            </div>
          </div>
