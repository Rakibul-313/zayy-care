...
function ShopProductCard({
  ...
  const [imageSource, setImageSource] = useState(safeImage(product.image));
  const isOutOfStock = Number(product.stock || 0) <= 0;
  const productHref = `/product/${product.slug || product.id}`;

  useEffect(() => {
    setImageSource(safeImage(product.image));
  }, [product.image]);
    ...
    <article className="group relative w-full max-w-full min-w-0 overflow-hidden rounded-[8px] border border-[#e8e3d7] bg-white shadow-[0_10px_28px_rgba(11,61,46,0.09)] transition hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(11,61,46,0.14)] sm:rounded-[12px]">
      <div className="relative">
        ...
        >
          <Image
            ...
            quality={80}
            onError={() => setImageSource("/products/p1.png")}
            className="object-contain p-3 transition duration-500 group-hover:scale-105 sm:object-cover sm:p-0"
          />
        </Link>
        ...
        >
          {isOutOfStock ? "OUT OF STOCK" : getBadge(product)}
        </span>
        ...
        >
          <Heart
            ...
            className={liked ? "text-red-500" : "text-[#0b3d2e]"}
          />
        </button>
      ...
      <div className="p-2 sm:p-3">
        <p className="mb-1 truncate text-[8px] font-black uppercase tracking-wide text-[#4f7a3a] sm:text-[10px]">
          {product.brand || product.category || "International Skincare"}
        </p>
        ...
        <button
          ...
          className={`mt-2.5 flex h-8 w-full items-center justify-center gap-1.5 rounded-[6px] text-[9px] font-black uppercase transition sm:mt-3 sm:h-9 sm:gap-2 sm:text-[12px] ${
            isOutOfStock
              ? "cursor-not-allowed bg-[#d7d9d5] text-[#6b7568] shadow-none"
              : "bg-[linear-gradient(90deg,#062a18_0%,#0b3d2e_50%,#062a18_100%)] text-white shadow-[0_8px_18px_rgba(11,61,46,0.20)] hover:bg-[#062a18]"
          }`}
        >
          {isOutOfStock ? (
            "Out of Stock"
...
function ShopProductSkeleton() {
  return (
    <div className="overflow-hidden rounded-[8px] border border-[#e8e3d7] bg-white sm:rounded-[12px]">
      <div className="aspect-square animate-pulse bg-[#f5f1e8] sm:aspect-[27/23]" />
      <div className="space-y-3 p-2 sm:p-3">
        ...
        <div className="h-5 w-20 animate-pulse rounded bg-[#e7e8e3]" />
        <div className="h-8 w-full animate-pulse rounded-[6px] bg-[#e7e8e3] sm:h-9" />
      </div>
    </div>
  );
}
      ...
      const formatted: ShopProduct[] = Object.entries(data)
        .map(([firebaseId, value], index) => {
          ...
          return {
            ...
            category: product.category || "International Skincare",
      ...
      >
            ...
            <div className="mx-auto max-w-[1820px] px-4 py-12 sm:px-8 md:py-16 lg:px-14 lg:py-20">
              <div className="relative z-10 max-w-[560px]">
                ...
                <p className="mt-4 max-w-[460px] text-[16px] leading-8 text-[#263421] sm:text-[17px]">
                  Discover carefully selected skincare and beauty products from trusted international brands.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="px-3 pb-10 sm:px-8 lg:px-14">
          <div className="mx-auto grid max-w-[1820px] grid-cols-2 gap-3 rounded-[6px] border border-[#0b3d2e]/10 bg-[#f5f1e8] p-4 lg:grid-cols-5">
            {[
              [ShieldCheck, "Carefully Selected", "International beauty products"],
              [Truck, deliveryTitle, deliveryText],
              [ShoppingBag, "Secure Checkout", "Safe payment options"],
              [RefreshCcw, "Return Support", "Simple return assistance"],
